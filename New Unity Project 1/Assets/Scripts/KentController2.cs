...
public class KentController2 : MonoBehaviour
{
    ...
    private OTAnimatingSprite kentsSprite;
    ...
    public enum anim { None, WalkLeft, WalkRight, StandLeft, StandRight, HangLeft, HangRight};
    ...
    void Start()
    {
        ...
        grounded = false;
        print("kent 2 start");
        kentsSprite = transform.Find("Kent's Sprite").GetComponent<OTAnimatingSprite>();

        //print(sprite.transform.position);

        
        kentsSprite.onCollision = OnCollision;
        if(attackBlock!=null)
    ...
    void Update()
    {
        ...
        float direction = Input.GetAxis("Horizontal");
            ...
            if (currentAnim != anim.WalkLeft && direction < 0)
            {
                currentAnim = anim.WalkLeft;
                kentsSprite.flipHorizontal = true;
                //kentsSprite.PlayOnLoop("run");
                kentsSprite.PlayLoop("run");
            }
            if (currentAnim != anim.WalkRight && direction > 0)
            {
                currentAnim=anim.WalkRight;
                kentsSprite.flipHorizontal = false;
                kentsSprite.PlayLoop("run");
            }
            if(direction==0){
              kentsSprite.Stop();
